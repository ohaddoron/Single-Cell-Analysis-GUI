...
try
    load('Control.mat');
    t = strrep(datestr(datetime('now')),':','-');
    status = copyfile(strcat(pwd,'\Control\Control.mat'),strcat(pwd,'\Control\Old\Control  ',t,'.mat'));
    if status == 0
        errordlg('Unable to make a copy of the control file');
    end
    
catch
     warndlg('No control file found. Creating new control file');
     Control = [];
     mkdir(strcat(pwd,'\Control\Old'));
     save(strcat(pwd,'\Control\Control.mat'),'Control');
end
rmpath(genpath(pwd));
...
handles.Version = get(hObject,'String');
guidata(hObject,handles);
...
addpath(genpath(pwd));
load('Control.mat');
Control.Version = handles.Version;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
handles.Version_Date = get(hObject,'String');
guidata(hObject,handles);
...
addpath(genpath(pwd));
load('Control.mat');
Control.Version_Date = handles.Version_Date;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
handles.Location_of_PPT_Templates = folderPath{1};
guidata(hObject,handles);
...
addpath(genpath(pwd));
load('Control.mat');
Control.Location_of_PPT_Templates = handles.Location_of_PPT_Templates;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
handles.Location_of_Layout_Excels = folderPath{1};
guidata(hObject,handles);
...
addpath(genpath(pwd));
load('Control.mat');
Control.Location_of_Layout_Excels = handles.Location_of_Layout_Excels;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
handles.Location_of_Outputs = folderPath{1};
guidata(hObject,handles);
...
addpath(genpath(pwd));
load('Control.mat');
Control.Location_of_Outputs = handles.Location_of_Outputs;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
try
    handles.normalization_parameters = [handles.normalization_parameters; pars];
catch
    handles.normalization_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
...
pars = contents(get(hObject,'Value'));
try
    handles.time_dependency_parameters = [handles.time_dependency_parameters; pars];
catch
    handles.time_dependency_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox13,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.maps_parameters = [handles.maps_parameters; pars];
catch
    handles.maps_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox14,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.maps_parameters_3D = [handles.maps_parameters_3D; pars];
catch
    handles.maps_parameters_3D = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox15,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.bar_graph_parameters = [handles.bar_graph_parameters; pars];
catch
    handles.bar_graph_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox16,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.layers_parameters = [handles.layers_parameters; pars];
catch
    handles.layers_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox17,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.par1_1_parameters = [handles.par1_1_parameters; pars];
catch
    handles.par1_1_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox18,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.par2_1_parameters = [handles.par2_1_parameters; pars];
catch
    handles.par2_1_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox19,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.par1_2_parameters = [handles.par1_2_parameters; pars];
catch
    handles.par1_2_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox20,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.par2_2_parameters = [handles.par2_2_parameters; pars];
catch
    handles.par2_2_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox21,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.intensity_parameters = [handles.intensity_parameters; pars];
catch
    handles.intensity_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox25,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.summary_table_parameters = [handles.summary_table_parameters; pars];
catch
    handles.summary_table_parameters = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox26,'String',contents)
guidata(hObject,handles);
...
pars = contents(get(hObject,'Value'));
try
    handles.summary_table_parameters_3D = [handles.summary_table_parameters_3D; pars];
catch
    handles.summary_table_parameters_3D = pars;
end
contents(strcmp(contents,pars)) = [];
set(handles.listbox27,'String',contents)
guidata(hObject,handles);
...
addpath(genpath(pwd));
load('Control.mat');
Control.normalization_parameters = handles.normalization_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control');
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.time_depedency_parameters = handles.time_depedency_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.maps_parameters = handles.maps_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.maps_parameters_3D = handles.maps_parameters_3D;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.bar_graph_parameters = handles.bar_graph_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.layers_parameters = handles.layers_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.par1_1_parameters = handles.par1_1_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.par2_1_parameters = handles.par2_1_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.par1_2_parameters = handles.par2_1_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.par2_2_parameters = handles.par2_2_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.intensity_parameters = handles.intensity_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.summary_table_parameters = handles.summary_table_parameters;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
...
addpath(genpath(pwd));
load('Control.mat');
Control.summary_table_parameters_3D = handles.summary_table_parameters_3D;
save(strcat(pwd,'\Control\Control.mat'),'Control'); 
rmpath(genpath(pwd));
