function Velocity_YVsVelocity_ZVsTime(folderPath,concat)
    ...
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Time']);
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Abs']);
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Sun']);
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Avg']);
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Time']);
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Abs']);
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Sun']);
    mkdir([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Avg']);
    ...
    %savePath = [folderPath '\Velocity_Y VsVelocity_Y Vs Time'];       
  ...
  par = {'dt','Velocity_X','Velocity_Y','Velocity_Z','x_Pos','y_Pos','z_Pos'};
    ...
    for i = 1 : len
        ...
        graphs_data.title{i} = strrep(strrep(files(i).name,'NNN0',''),'.mat','');
        ...
        %r(r==0) = 1;
        %{
        [~, firstIdxs] = max(~isnan(data.Velocity_Y));
        Vxfirst = diag(data.Velocity_Y(firstIdxs,1:size(data.Velocity_Y,2)));
        Vyfirst = diag(data.Velocity_Y(firstIdxs,1:size(data.Velocity_Y,2)));
        [Vxtottemp,~] = meshgrid(Vxfirst,1:size(data.Velocity_Y,1));
        [Vytottemp,~] = meshgrid(Vyfirst,1:size(data.Velocity_Y,1));
        Vxtottemp = data.Velocity_Y - Vxtottemp;
        Vytottemp = data.Velocity_Y - Vytottemp;
        %}
        Vxtot{i} = data.Velocity_Y(:);
        Vytot{i} = data.Velocity_Z(:);
        ...
        Vxtime{i} = [Vxtime{i} nanmean(abs(data.Velocity_Y),2)];
        Vytime{i} = [Vytime{i} nanmean(abs(data.Velocity_Z),2)];
    ...
    for i = 1 : len
        ...
        xlabel(['Velocity X ' Units('Velocity_Y')]);
        ylabel(['Velocity_Z ' Units('Velocity_Z')]);
        hcolor = colorbar;
        xlabel(hcolor,'Time [min]');
        ax = gca;
        maxax = max([ax.XLim,ax.YLim]);
        axis([-Ax Ax -Ax Ax]);
        saveas(handler(i),[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Sun\' graphs_data.title{i}]);
        saveas(handler(i),[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Sun\' graphs_data.title{i} '.tiff']);
        ...
        title([graphs_data.title{i}]);
        xlabel(['Velocity X ' Units('Velocity_Y')]);
        ylabel(['Velocity Y ' Units('Velocity_Z')]);
        hcolor = colorbar;
        xlabel(hcolor,'Time [min]');
        ax = gca;
        maxax = max([ax.XLim,ax.YLim]);
        axis([0 Ax 0 Ax]);
        saveas(handler(i),[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Abs\' graphs_data.title{i}]);
        saveas(handler(i),[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Abs\' graphs_data.title{i} '.tiff']);
        close all;
        ...
        title([graphs_data.title{i} ' time']);
        xlabel(['Velocity X ' Units('Velocity_Y')]);
        ylabel(['Velocity Y ' Units('Velocity_Z')]);
        hcolor = colorbar;
        xlabel(hcolor,'Time [min]');
        ...
        maxax = max([ax.XLim,ax.YLim]);
        axis([0 Ax/5 0 Ax/5]);
        saveas(handler(i),[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Time\' graphs_data.title{i}]);
        saveas(handler(i),[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Time\' graphs_data.title{i} '.tiff']);
    ...
    xlswrite([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Graphs Data'],Data,'Avg over Time');
    t1 = {'Vx'};
    t2 = {'Vy'};
    headers = [[graphs_data.title nan graphs_data.title]; [repmat(t1,[1 size(Vxtime,2)]) nan repmat(t2,[1 size(Vxtime,2)])]];
    xlswrite([folderPath '\Velocity_Y VsVelocity_Z Vs Time\Graphs Data'],headers,'Avg over Time')
    title('Vy vs Vz');
    xlabel(['Velocity Y ' Units('Velocity_Y')]);
    ylabel(['Velocity Z ' Units('Velocity_Z')]);
    axis([0 Ax/4 0 Ax/4]);
    legend(legends,'Location','northwestoutside');
    %toPPT(h,'SlideNumber',3);
    saveas(h,[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Avg\Vx vs Vy']);
    saveas(h,[folderPath '\Velocity_Y VsVelocity_Z Vs Time\Images\Avg\Vx vs Vy.tiff']);
