function descreptiveStatistics(folder_path,par,TP)
    ...
    for j = 1:length(par)
        for i=1:num_of_mat_files      
            ...
            if TP.choice == 0
                ...
                dt = data_struct.dt;
                low = round(TP.low/dt) + 1;
                high = round(TP.high/dt) + 1;
%                 TP.low = round(TP.low / data_struct.dt);
%                 TP.high = round(TP.high / data_struct.dt);
                YPos = data_struct.y_Pos(low:high,:);
                Val = data_struct.(par{j})(low:high,:);
            end
            t = (0:size(Val,1)-1)';
            for k = 1 : size(Val,2)
                Value = Val(:,k);
