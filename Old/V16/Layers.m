...
%             time = meshgrid(t,1:l)';
            ...
            for k = 1 : size(graphs_data.plot_data.(graphs_data.title{i}).(par{j}).Mean,2)
...
%                     graphs_data.plot_data.(graphs_data.title{i}).(par{j}).STDEV(:,k),'Color',c(k,:));
                if i == 1
                    if k < size(graphs_data.plot_data.(graphs_data.title{i}).(par{j}).Mean,2)
            ...
            data = flipud(graphs_data.plot_data.(graphs_data.title{i}).(par{j}).Mean');
            t(find(sum(isnan(data)))) = [];
            data(:,find(sum(isnan(data)))) = [];
            imagesc(data);
            axis tight;
            set(gca,'YTick',1:size(data,1));
            set(gca,'YTickLabel',graphs_data.legend);
            try
                if ~isemtpy(t)
                    set(gca,'XTickLabel',t(get(gca,'XTick')))
                end
            catch
                set(gca,'XTickLabel',t*tint);
            end
            colormap(jet);
            hold off;
            cax = [cax; caxis];
            ...
            %}
            title([graphs_data.title{i} ' - ' par{j}]);
            xlabel('Time [min]');
            b = colorbar;
            xlabel(b,[par{j} ' ' Units(par{j})]);
...
%     maxax = -inf;
    ...
    for j = 1 : numel(par)
        ...
        for i = 1 : num_of_mat_files
            set(0, 'currentfigure', handler.(par{j})(i));
            caxis([cmin cmax]);
            saveas(handler.(par{j})(i),[folder_path '\Layers\Images\' par{j} '\' graphs_data.title{i} ' - ' par{j} '.tiff']);
            saveas(handler.(par{j})(i),[folder_path '\Layers\' par{j} '\' graphs_data.title{i} ' - ' par{j}]);
            close(handler.(par{j})(i));
        end
    end
    close all;
    warning('on','all');
