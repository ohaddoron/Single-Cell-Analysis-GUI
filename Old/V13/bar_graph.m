function handler = bar_graph(varargin)
    ...
    for i = 1 : length(par)
        ...
        for j = 1 : numel(graphs_data.labels.(par{i}))
            graphs_data.labels.(par{i}){j} = [graphs_data.labels.(par{i}){j}(12:14) graphs_data.labels.(par{i}){j}(19:end)];
        end
        ...
        title(['Average cell ' strrep(par{i},'_',' ')],'FontSize',16);
        flag = 1;
...
%             try
%                 
%                 xlswrite([folder_path '\Bar Graphs\Tukey Table.xlsx'],[nan(1,size(c.(par{i}),2)); c.(par{i})],par{i});
%                 xlswrite([folder_path '\Bar Graphs\Tukey Table.xlsx'],[cell(1,size(names,2)); names],par{i});
%                 xlswrite([folder_path '\Bar Graphs\Tukey Table.xlsx'],{'','','Lower Confidence Interval','Estimate','Upper Confidence Interval','p-value'},par{i});
%                
%             catch
%             end
%         end
