function [handler,Ax] = MD_parameter_to_time_multiple(folder_path,par,scale,Arrange,concat)
    ...
    for i=1:len
        ...
        if concat ~= 0
            ...
            graphs_data.legend{i} = strrep(strrep(strrep([files(i).name(1:5) files(i).name(12:14) files(i).name(16:end)],'NNN0',''),'.mat',''),'_',' ');
        end
        for j = 1 : length(par)
            if scale.choice(1) == 0
    ...
    for i = 1 : length(fieldnames(graphs_data.plot_data))
        ...
        for j = 1 : size(c,1)
            errorbar(t(:,j),graphs_data.plot_data.(par{i})(:,j),...
                graphs_data.plot_error.(par{i})(:,j),'Color',c(j,:));
        end
        %errorbar(t,graphs_data.plot_data.(par{i}),graphs_data.plot_error.(par{i}));
        handler_legend = legend(graphs_data.legend,'Location','northwestoutside');
