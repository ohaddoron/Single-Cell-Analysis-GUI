function Renaming(folderPath,outputLocation,xlsTemplatePath)
    ...
    for k = 1 : length(files)
        ...
        if remj >= 10
            EXPname = cell2mat([raw(2,1:5) char(remi) num2str(remj) raw((find(rowLoc,1,'first'):find(rowLoc,1,'first')+5),find(colLoc,1,'first')+2)']);
        else
            EXPname = cell2mat([raw(2,1:5) char(remi) '0' num2str(remj) raw((find(rowLoc,1,'first'):find(rowLoc,1,'first')+5),find(colLoc,1,'first')+2)']);
        end
