function varargout = data2ppt_GUI(varargin)
% DATA2PPT_GUI MATLAB code for data2ppt_GUI.fig
%      DATA2PPT_GUI, by itself, creates a new DATA2PPT_GUI or raises the existing
%      singleton*.
%
%      H = DATA2PPT_GUI returns the handle to a new DATA2PPT_GUI or the handle to
%      the existing singleton*.
%
%      DATA2PPT_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DATA2PPT_GUI.M with the given input arguments.
%
%      DATA2PPT_GUI('Property','Value',...) creates a new DATA2PPT_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before data2ppt_GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to data2ppt_GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help data2ppt_GUI

% Last Modified by GUIDE v2.5 28-Feb-2016 16:19:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @data2ppt_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @data2ppt_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before data2ppt_GUI is made visible.
function data2ppt_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to data2ppt_GUI (see VARARGIN)

% Choose default command line output for data2ppt_GUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes data2ppt_GUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = data2ppt_GUI_OutputFcn(hObject, eventdata, handles) 
...
handles.imarisdirs{1} = uigetdir2(Control.Location_of_Outputs,'Select Imaris folder');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.FullRename = fullfile(handles.PathReName,handles.FileReName);
...
%}
data2ppt(handles.imarisdirs,handles.FullRename,handles.FullLayName,handles.outRename,handles.matdirs,handles.datadirs,cell2mat(handles.int),cell2mat(handles.ss),cell2mat(handles.IntensityAnalysis),cell2mat(handles.Concatanate),cell2mat(handles.ND),cell2mat(handles.normalize));
...
[handles.FileReName{1},handles.PathReName{1}] = uigetfile('*.*','Choose file to load :',Control.Location_of_Layout_Excels);
...
[handles.FileLayName{1},handles.PathLayName{1}] = uigetfile('*.*','Choose file to load :',Control.Location_of_Layout_Excels);
for i = 2:handles.loop
    [handles.FileLayName{i},handles.PathLayName{i}] = uigetfile('*.*','Choose file to load :',handles.PathLayName{i-1});
end
set(handles.LayoutList,'String',handles.FileLayName);
rmpath(genpath(pwd));
guidata(hObject,handles);
...
handles.datadirs{1} = uigetdir2(handles.outpathstr,'Select output directory for the data');
[handles.outpathstr,~,~] = fileparts(char(handles.outRename{1}));
for i = 1:handles.loop-1
    handles.datadirs{i+1} = uigetdir2(handles.outpathstr,'Select output directory for the data');
end
for i = 1:handles.loop
...
handles.outRename{1} = uigetdir2(Control.Location_of_Outputs,'Choose directory');
[handles.outpathstr,~,~] = fileparts(char(handles.outRename{1}));
for i = 1:handles.loop-1
...
% handles    structure with handles and user data (see GUIDATA)
for i = 1:handles.loop
    prompt = 'Enter 1 if you wish to concatanate, enter 0 otherwise';
    title = 'Concatanate';
    answer = inputdlg(prompt,title);
    handles.Concatanate{i} = str2double(answer{1});
end
set(handles.listbox11,'String',handles.Concatanate');
guidata(hObject,handles);
