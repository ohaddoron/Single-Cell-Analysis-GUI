...
%                 set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
                ...
                title(cell2mat([par2(k) ' vs ' par1(j) ' Sun - ' titleName]));
                savefig(h,[folderPath '\par1_vs_par2\' par1{j} '\' par2{k} '\Sun\' strrep(strrep(files(i).name,'NNN0',''),'.mat','') '.fig']);
                xls_write_data = [{par1{j}, par2{k}, 'Time'}; num2cell(curData)];
                try
                    xlswrite(fullfile(folderPath,'par1_vs_par2',par1{j},par2{k},'Graphs Data - Sun.xlsx'),xls_write_data,titleName);
                catch
                    errordlg(['Excel not created in parameter correlation ' strrep(par2{k},'_',' ') ' vs ' strrep(par1{j},'_',' ') ' - Sun']);
                end
...
%                 set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
                ...
                title(cell2mat([par2(k) ' vs ' par1(j) ' Absolute - ' titleName]));
                savefig(h,[folderPath '\par1_vs_par2\' par1{j} '\' par2{k} '\Abs\' strrep(strrep(files(i).name,'NNN0',''),'.mat','') '.fig']);
                xls_write_data = [{par1{j}, par2{k}, 'Time'}; num2cell(curData)];
                try
                    xlswrite(fullfile(folderPath,'par1_vs_par2',par1{j},par2{k},'Graphs Data - Abs.xlsx'),xls_write_data,titleName);
                catch
                    errordlg(['Excel not created in parameter correlation ' strrep(par2{k},'_',' ') ' vs ' strrep(par1{j},'_',' ') ' - Abs']);
                end
...
%                 set(gcf, 'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
                ...
                xlabel(strrep([par1{j} ' ' Units(par1{j})],'_',' '));
                ylabel(strrep([par2{k} ' ' Units(par2{k})],'_',' '));
                title(cell2mat([par2(k) ' vs ' par1(j) ' Average over time - ' titleName]));
                savefig(h,[folderPath '\par1_vs_par2\' par1{j} '\' par2{k} '\Time\' strrep(strrep(files(i).name,'NNN0',''),'.mat','') '.fig']);
                xls_write_data = [{par1{j}, par2{k}, 'Time'}; num2cell(meanData)];
                try
                    xlswrite(fullfile(folderPath,'par1_vs_par2',par1{j},par2{k},'Graphs Data - Time.xlsx'),xls_write_data,titleName);
                catch
                    errordlg(['Excel not created in parameter correlation ' strrep(par2{k},'_',' ') ' vs ' strrep(par1{j},'_',' ') ' - Avg']);
                end
...
%             h = figure;
            ...
            for m = 1 : size(data_struct.par1.(par1{j}),2)
              ...
              mm = mm + 1;
              xls_write_data = [data_struct.par1.(par1{j})(:,m) data_struct.par2.(par2{k})(:,m) t'];
              try
                xlswrite(fullfile(folderPath,'par1_vs_par2',par1{j},par2{k},'Graphs Data - Avg.xlsx'),xls_write_data,legends{i});
              catch
                errordlg(['Excel not created in parameter correlation ' strrep(par2{k},'_',' ') ' vs ' strrep(par1{j},'_',' ') ' - Avg']);
              end
            end
