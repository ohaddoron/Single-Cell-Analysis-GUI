function [handler,Ax] = Time_Dependency_Maps(folder_path,par,scale,Arrange,concat,varargin)
    ...
    if nargin > 6 
        Order_xls = varargin{2};
        if ~isempty(Order_xls)
            [~,~,raw] = xlsread(Order_xls);
            idx = strfind(raw(1,:),'Order');
            cidx = find(~cellfun(@isempty,idx));
            Arrange.list = raw(2:end,cidx(1));
            Arrange.choice = 0;

            idx = strfind(raw(1,:),'Final Name');
            cidx = find(~cellfun(@isempty,idx));
        end
    ...
    for i = 1 : length(fieldnames(graphs_data.plot_data))
        if size(graphs_data.plot_data.(par{i}),1) > 1
            if Arrange.choice == 0
                ...
                graphs_data.legend = flipud(raw(2:end,cidx));
