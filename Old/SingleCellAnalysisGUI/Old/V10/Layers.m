...
%         close all;
    ...
    for j = 1 : numel(par)
        for i = 1 : num_of_mat_files
            maxax = max([maxax,handler.(par{j})(i).CurrentAxes.YLim]);
            minax = min([minax,handler.(par{j})(i).CurrentAxes.YLim]);
        end
        for i = 1 : num_of_mat_files
            handler.(par{j})(i).CurrentAxes.YLim = [minax maxax];
            handler.(par{j})(i).CurrentAxes.XLim = [0 max(max(t))];
            saveas(handler.(par{j})(i),[folder_path '\Layers\Images\' par{j} '\' graphs_data.title{i} ' - ' par{j} '.tiff']);
            saveas(handler.(par{j})(i),[folder_path '\Layers\' par{j} '\' graphs_data.title{i} ' - ' par{j}]);
        end
    end
