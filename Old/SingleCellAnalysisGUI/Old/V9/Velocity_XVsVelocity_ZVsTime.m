function Velocity_XVsVelocity_ZVsTime(folderPath,concat)
    ...
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Time']);
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Abs']);
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Sun']);
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Avg']);
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Time']);
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Abs']);
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Sun']);
    mkdir([folderPath '\Velocity_X VsVelocity_Z Vs Time\Avg']);
    ...
    for i = 1 : len
        ...
        axis([-Ax Ax -Ax Ax]);
        saveas(handler(i),[folderPath '\Velocity_X VsVelocity_Z Vs Time\Sun\' graphs_data.title{i}]);
        saveas(handler(i),[folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Sun\' graphs_data.title{i} '.tiff']);
        ...
        maxax = max([ax.XLim,ax.YLim]);
        axis([0 Ax 0 Ax]);
        saveas(handler(i),[folderPath '\Velocity_X VsVelocity_Z Vs Time\Abs\' graphs_data.title{i}]);
        saveas(handler(i),[folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Abs\' graphs_data.title{i} '.tiff']);
        close all;
        ...
        maxax = max([ax.XLim,ax.YLim]);
        axis([0 Ax/5 0 Ax/5]);
        saveas(handler(i),[folderPath '\Velocity_X VsVelocity_Z Vs Time\Time\' graphs_data.title{i}]);
        saveas(handler(i),[folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Time\' graphs_data.title{i} '.tiff']);
    ...
    xlswrite([folderPath '\Velocity_X VsVelocity_Z Vs Time\Graphs Data'],Data,'Avg over Time');
    t1 = {'Vx'};
    t2 = {'Vy'};
    headers = [[graphs_data.title nan graphs_data.title]; [repmat(t1,[1 size(Vxtime,2)]) nan repmat(t2,[1 size(Vxtime,2)])]];
    xlswrite([folderPath '\Velocity_X VsVelocity_Z Vs Time\Graphs Data'],headers,'Avg over Time')
    title('Vx vs Vz');
    xlabel(['Velocity X ' Units('Velocity_X')]);
    ylabel(['Velocity Y ' Units('Velocity_Z')]);
    axis([0 Ax/4 0 Ax/4]);
    ...
    saveas(h,[folderPath '\Velocity_X VsVelocity_Z Vs Time\Avg\Vx vs Vy']);
    saveas(h,[folderPath '\Velocity_X VsVelocity_Z Vs Time\Images\Avg\Vx vs Vy.tiff']);
