function Cluster( xlsPath , folderPath , outputLocation, single )
...
%     data = data - repmat(nanmean(data),size(data,1),1);
    ...
    StandardizeValue = 0;
    if nargin >=4 && single ~= 0
        CGobj = clustergram(data,'ColumnLabels',pars,'Standardize',StandardizeValue);
    else
...
%                                             'ColumnLabels',pars,'Standardize',StandardizeValue);
    ...
    saveas(h,[imPath '\Clustergram.tiff']);
    savefig(h,[figPath '\Clustergram.fig']);
