function handler = scatter_plot_position_2_time_3D_multiple_par(varargin)
    ...
    for i=1:num_of_mat_files
        ...
        for j = 1:length(par)
            ...
            nan_idx = graphs_data.plot_data.y_Pos{i}<0;
            graphs_data.plot_data.y_Pos{i}(nan_idx) = nan;
            ...
            graphs_data.plot_data.(par{j}){i}(nan_idx) = nan;
    ...
    for i = 1 : num_of_mat_files
        ...
        for j = 1 : length(par)
            ...
            title([graphs_data.title.(par{j}){i}(1:5) graphs_data.title.(par{j}){i}(12:end)],'FontSize',16);
