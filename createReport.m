...
% % Load existing report
    addpath(genpath(pwd));
    load('Control.mat');
    Path = strcat(Control.Report_Path,'\Report.mat');
    try
        
        load(Path);
        a = whos('Report');
        if a.bytes > 5e6
            mkdir(strcat(Control.Report_Path,'\Old'));
            num_of_reports = numel(dir(strcat(Control.Report_Path,'\Old\*.mat')));
            movefile(Path,[Control.Report_Path '\Old\Report ' num2str(num_of_reports + 1) '.mat']);
            error;
        end
    catch
        Report = cell(1,46);
